...
// useOnlineStatus.js), which scattered raw `axios` calls could never provide. Also handles the
// native app's offline needs: caches GET responses (offlineCache.js) to serve back when a
// request fails with no network, and queues PUT/PATCH/DELETE requests (offlineQueue.js) for
// automatic replay on reconnect — see the response interceptor below. Requires
// public/service-worker.js to NOT swallow /api/ network failures into a fake response (it
// used to), since that would prevent axios from ever seeing a true network error here.
...
// via apiClient(config) exactly like the original call site invoked it.
...
apiClient.interceptors.response.use(
    ...
    async (error) => {
        if (!error.response) {
            ...
            // optimistically here so call sites behave the same as a normal success (many
            // already apply their own optimistic local state update before the request; those
            // that instead rely on `response.data` being the full updated document should treat
            // it as a merge, not a replace — this synthetic response only echoes the sent
            // payload, not a server-computed document).
