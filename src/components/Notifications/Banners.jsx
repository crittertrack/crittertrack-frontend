...
const UrgencyAlertsBanner = ({ authToken, API_BASE_URL }) => {
    ...
    const typeConfig = {
        mated:  { label: 'Mating',         bg: 'bg-sky-100 text-sky-700', icon: '\u2665\uFE0F' },
        due:    { label: 'Expected Birth', bg: 'bg-amber-100 text-amber-700',   icon: '\uD83D\uDC23' },
        weaned: { label: 'Weaning',        bg: 'bg-blue-100 text-blue-700',       icon: '\uD83C\uDF7C' },
    };
...
const MgmtUrgencyBanner = ({ authToken, API_BASE_URL }) => {
    ...
    // Animal care tasks (free-form array + the 14 dedicated Grooming/Special Care & Training schedule fields)
    let careDueCount = 0;
    animals.forEach(a => {
