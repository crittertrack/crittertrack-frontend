...
const FamilyTreePage = ({ API_BASE_URL, authToken, myAnimals, onViewAnimal }) => {
    ...
    return (
        <div className="p-4 sm:p-6 bg-white dark:bg-dark-card-bg rounded-xl shadow-lg">
            ...
            <div className="max-w-lg mx-auto mb-8 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select
                        ...
                        className="w-full py-2.5 px-3 border border-gray-300 dark:border-dark-text rounded-lg bg-white dark:bg-dark-card-bg text-gray-900 dark:text-dark-text focus:outline-none focus:ring-2 focus:ring-primary"
                    >
                ...
                <div>
                    ...
                    <div className="relative" ref={searchRef}>
                        <div className="flex items-center border border-gray-300 dark:border-dark-text rounded-lg bg-white dark:bg-dark-card-bg">
                            <Search className="h-5 w-5 text-gray-400 dark:text-dark-text-muted mx-3" />
                            <input
                                id="animal-search"
                                type="text"
                                value={searchTerm}
                                ...
                                className="w-full py-2.5 pr-10 bg-transparent focus:outline-none text-gray-900 dark:text-dark-text dark:placeholder-dark-text-muted disabled:bg-gray-100 dark:disabled:bg-dark-surface disabled:cursor-not-allowed"
                            />
                            {searchTerm && (
                                <button onClick={() => { setSearchTerm(''); setSelectedAnimal(null); }} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 dark:text-dark-text-muted hover:text-gray-700 dark:hover:text-dark-text">
                                    <X size={18} />
                                </button>
                            )}
                        </div>
                        {isDropdownOpen && selectedSpecies && (
                            <div className="absolute z-10 mt-1 w-full bg-white dark:bg-dark-card-bg border border-gray-200 dark:border-dark-text rounded-lg shadow-lg max-h-60 overflow-y-auto">
                                ...
                                ) : (<div className="px-4 py-2 text-gray-500 dark:text-dark-text-muted">No animals found.</div>)}
                            </div>
                        )}
                    </div>
                </div>
            ...
            <div className="max-w-lg mx-auto mb-8">
                ...
                <div className="flex rounded-lg border border-gray-300 dark:border-dark-text p-1 bg-gray-200 dark:bg-dark-card-bg">
                    <button 
                        ...
                        className={`w-1/2 p-2 rounded-md text-sm font-semibold flex items-center justify-center gap-2 transition ${graphMode === 'direct' ? 'bg-white dark:bg-dark-surface shadow dark:text-dark-text' : 'text-gray-600 dark:text-dark-text-secondary hover:bg-gray-300 dark:hover:bg-dark-surface-hover'}`}
                    >
                        Direct Lines
                    </button>
                    <button 
                        onClick={() => setGraphMode('full')}
                        className={`w-1/2 p-2 rounded-md text-sm font-semibold flex items-center justify-center gap-2 transition ${graphMode === 'full' ? 'bg-white dark:bg-dark-surface shadow dark:text-dark-text' : 'text-gray-600 dark:text-dark-text-secondary hover:bg-gray-300 dark:hover:bg-dark-surface-hover'}`}
                    >
                        Full Graph
                    </button>
                </div>
            ...
            <div className="mt-8 border-t border-gray-200 dark:border-dark-text pt-8" style={{ height: '70vh', width: '100%', overflow: 'hidden' }}>
                {selectedAnimal ? (
                    <FamilyTreeView 
                        animals={myAnimals} 
