...
const STATUS_CONFIG = {
    ...
    dismissed: { icon: XCircle, label: 'Dismissed', className: 'bg-gray-100 text-gray-600 dark:bg-dark-card-bg dark:text-dark-text-muted' },
};
...
export default function ReportPage({ authToken, userProfile, showModalMessage }) {
                ...
                <div className="space-y-2">
                    {myReportsLoading ? (
                        ...
                        <div className="text-center py-10 px-4 bg-white dark:bg-dark-card-bg rounded-lg border border-dashed border-gray-200 dark:border-dark-text-muted">
                            <p className="text-sm text-gray-500 dark:text-dark-text-muted">You haven't submitted any reports yet.</p>
                        </div>
                    ) : (
                        myReports.map(r => {
                            ...
                            return (
                                <div key={r._id} className="bg-white dark:bg-dark-card-bg rounded-lg border border-gray-200 dark:border-dark-text-muted p-3">
                                    <div className="flex items-center justify-between gap-2 mb-1">
                                        <span className="text-xs font-semibold text-gray-500 dark:text-dark-text-muted">{r.category} <span className="font-normal text-gray-400 dark:text-dark-text-muted">#{r._id.slice(-6).toUpperCase()}</span></span>
                                    ...
                                    {r.subjectLabel && (
                                        <p className="text-xs font-medium text-gray-500 dark:text-dark-text-muted mb-1">{r.subjectLabel}</p>
                                    )}
                                    {r.referenceId && (
                                        <p className="text-xs text-gray-400 dark:text-dark-text-muted mb-1">Ref: {r.referenceId}</p>
                                    )}
                                    <p className="text-sm text-gray-700 dark:text-dark-text-secondary whitespace-pre-wrap break-words">{r.description}</p>
                                    {r.stepsToReproduce && (
                                        <div className="mt-2 pt-2 border-t border-gray-100 dark:border-dark-text-muted">
                                            <p className="text-xs font-semibold text-gray-500 dark:text-dark-text-muted mb-1">Steps to Reproduce</p>
                                            <p className="text-sm text-gray-700 dark:text-dark-text-secondary whitespace-pre-wrap break-words">{r.stepsToReproduce}</p>
                                        </div>
                                    )}
                                    {r.images?.length > 0 && (
                                    ...
                                    <p className="text-xs text-gray-400 dark:text-dark-text-muted mt-1.5">{new Date(r.createdAt).toLocaleDateString()}</p>
                                </div>
                            );
                        })
                    )}
        ...
        return (
            ...
            <div className="bg-white dark:bg-dark-card-bg rounded-xl shadow-lg p-8">
                <div className="text-center py-8">
                    ...
                    <p className="text-gray-600 dark:text-dark-text-muted mb-1">
                    ...
                    <div className="flex gap-3 justify-center">
                        ...
                        <button
                            onClick={() => navigate('/')}
                            className="px-6 py-2.5 bg-gray-200 dark:bg-dark-card-bg text-gray-700 dark:text-dark-text-secondary font-semibold rounded-lg hover:bg-gray-300 dark:hover:bg-dark-surface-hover transition"
                        >
                            Back to Home
                        </button>
                    </div>
    ...
    return (
        <div className="w-full max-w-2xl mx-auto">
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                                ...
                                <button
                                    ...
                                    className={`p-4 rounded-xl border-2 text-left transition-all ${
                                        category === opt.value
                                            ...
                                            : 'border-gray-200 dark:border-dark-text-muted bg-gray-50 dark:bg-dark-card-bg/50 hover:border-gray-300 dark:hover:border-dark-text-muted'
                                    }`}
                                >
                                    <div className={`flex items-center gap-2 mb-1.5 ${
                                        category === opt.value ? 'text-purple-700 dark:text-purple-300' : 'text-gray-600 dark:text-dark-text-muted'
                                    }`}>
                                        {opt.icon}
                                        <span className="font-semibold text-sm">{opt.label}</span>
                                    </div>
                                    <p className="text-xs text-gray-500 dark:text-dark-text-muted">{opt.description}</p>
                                </button>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="flex gap-3 pt-2">
                        <button
                            ...
                            onClick={() => navigate(-1)}
                            disabled={submitting}
                            className="flex-1 px-4 py-2.5 bg-gray-200 dark:bg-dark-card-bg text-gray-700 dark:text-dark-text-secondary font-semibold rounded-lg hover:bg-gray-300 dark:hover:bg-dark-surface-hover transition disabled:opacity-50"
                        >
                            Cancel
                        </button>
                        <button
            ...
            {/* Privacy note */}
            <p className="text-xs text-gray-400 dark:text-dark-text-muted text-center mt-4 px-6">
