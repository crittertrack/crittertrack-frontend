...
const LocationManagerModal = ({ isOpen, onClose, locations, onSave, onDelete, saving }) => {
    ...
    return (
        <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/60" onClick={onClose}>
            <div className="bg-white dark:bg-dark-surface rounded-lg shadow-xl p-4 sm:p-6 w-full max-w-2xl max-h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}>
                <div className="flex justify-between items-center mb-4 pb-4 border-b dark:border-dark-border">
                    ...
                    <button onClick={onClose} className="p-1 rounded-full hover:bg-gray-100 dark:hover:bg-dark-surface-hover"><X size={22} /></button>
                </div>

                {formState ? (
                    // Form View
                    <div className="space-y-4">
                        <h4 className="font-semibold text-lg">{formState.id ? 'Edit' : 'Add'} {formState.type === 'building' ? 'Building' : 'Room'}</h4>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-dark-text-secondary">Name</label>
                            <input type="text" name="name" value={formState.name} onChange={handleFormChange} className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm dark:bg-dark-surface-hover dark:border-dark-border" />
                        </div>
                        {formState.type === 'room' && (
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-dark-text-secondary">Building</label>
                                <select name="parentLocationId" value={formState.parentLocationId} onChange={handleFormChange} className="mt-1 block w-full p-2 border border-gray-300 rounded-md shadow-sm dark:bg-dark-surface-hover dark:border-dark-border">
                        ...
                        {formState.type === 'building' && (
                            <div className="space-y-3 p-3 border rounded-md bg-gray-50 dark:bg-dark-surface-hover dark:border-dark-border">
                                <h5 className="font-medium text-gray-600 dark:text-dark-text">Address (Optional)</h5>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    <div><label className="text-xs text-gray-500">Street</label><input type="text" name="street" value={formState.address.street || ''} onChange={handleAddressChange} className="mt-1 block w-full p-2 text-sm border border-gray-300 rounded-md dark:bg-dark-bg dark:border-dark-border" /></div>
                                    <div><label className="text-xs text-gray-500">City</label><input type="text" name="city" value={formState.address.city || ''} onChange={handleAddressChange} className="mt-1 block w-full p-2 text-sm border border-gray-300 rounded-md dark:bg-dark-bg dark:border-dark-border" /></div>
                                    <div><label className="text-xs text-gray-500">State / Province</label><input type="text" name="state" value={formState.address.state || ''} onChange={handleAddressChange} className="mt-1 block w-full p-2 text-sm border border-gray-300 rounded-md dark:bg-dark-bg dark:border-dark-border" /></div>
                                    <div><label className="text-xs text-gray-500">Postal Code</label><input type="text" name="postalCode" value={formState.address.postalCode || ''} onChange={handleAddressChange} className="mt-1 block w-full p-2 text-sm border border-gray-300 rounded-md dark:bg-dark-bg dark:border-dark-border" /></div>
                                    <div className="sm:col-span-2"><label className="text-xs text-gray-500">Country</label><input type="text" name="country" value={formState.address.country || ''} onChange={handleAddressChange} className="mt-1 block w-full p-2 text-sm border border-gray-300 rounded-md dark:bg-dark-bg dark:border-dark-border" /></div>
                                </div>
                            </div>
                        )}
                    ...
                    <div className="flex-grow overflow-y-auto pr-2 space-y-4">
                        {buildings.map(building => {
                            ...
                            return (
                                <div key={building._id} className="bg-gray-50 dark:bg-dark-surface-hover p-3 rounded-lg">
                                    <div className="flex justify-between items-start">
                                        ...
                                        <div className="flex items-center gap-1 shrink-0">
                                            <button onClick={() => handleEdit(building)} className="p-1.5 rounded-full hover:bg-gray-200 dark:hover:bg-dark-border" title="Edit Building"><Edit size={14} /></button>
                                        ...
                                        {buildingRooms.map(room => (
                                            <div key={room._id} className="flex justify-between items-center text-sm">
                                                ...
                                                <div className="flex items-center gap-1">
                                                    <button onClick={() => handleEdit(room)} className="p-1 rounded-full hover:bg-gray-200 dark:hover:bg-dark-border"><Edit size={12} /></button>
                        ...
                        <button onClick={handleAddNewBuilding} className="w-full mt-4 px-4 py-2 text-sm font-semibold text-white bg-accent rounded-md hover:bg-accent/90 flex items-center justify-center gap-2"><Plus size={16} /> Add New Building</button>
                    </div>
                )}
            </div>
        </div>
