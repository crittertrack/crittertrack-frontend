...
const ToolsDropdown = ({ onLinkClick }) => {
  return (
    <div className="absolute top-full right-0 mt-2 w-56 rounded-md shadow-lg bg-white dark:bg-dark-card-bg ring-1 ring-black dark:ring-dark-text ring-opacity-5 focus:outline-none py-1 z-10">
      {toolLinks.map((link) => (
        <NavLink
          key={link.href}
          to={link.href}
          onClick={onLinkClick}
          className={({ isActive }) =>
            `flex items-center px-4 py-2 text-sm text-gray-700 dark:text-dark-text hover:bg-gray-100 dark:hover:bg-dark-surface-hover ${
