...
const TimelineEvent = ({ icon, title, date, children }) => (
    <div className="relative pl-10 pb-8 group">
        {/* Vertical line */}
        <div className="absolute top-2 left-4 -ml-px h-full w-0.5 bg-gray-200 group-last:hidden" />
        {/* Icon */}
...
export const TimelineTabContent = ({ animal }) => {
    const milestones = parseJsonField(animal.milestones).sort((a, b) => new Date(b.startDate) - new Date(a.startDate));
